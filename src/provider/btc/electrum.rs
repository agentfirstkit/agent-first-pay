...
impl ElectrumSource {
    ...
    fn make_client(&self) -> Result<BdkElectrumClient<electrum_client::Client>, PayError> {
        let inner = electrum_client::Client::new(&self.url)
            .map_err(|e| PayError::network_error(format!("electrum client: {e}")))?;
...
impl BtcChainSource for ElectrumSource {
    async fn sync(&self, wallet: &mut Wallet) -> Result<(), PayError> {
        ...
        let update = client
            ...
            .map_err(|e| PayError::network_error(format!("electrum sync: {e}")))?;
        wallet
            .apply_update(update)
    ...
    async fn full_scan(&self, wallet: &mut Wallet) -> Result<(), PayError> {
        ...
        let update = client
            ...
            .map_err(|e| PayError::network_error(format!("electrum full_scan: {e}")))?;
        wallet
            .apply_update(update)
