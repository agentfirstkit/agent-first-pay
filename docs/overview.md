...
See [Architecture](architecture.md) for advanced multi-server deployment patterns.
...
Other modes: `--mode interactive` (REPL), `--mode tui` (full-screen terminal UI), `--mode pipe` (JSONL stdin/stdout), `--mode rpc` (gRPC daemon), `--mode rest` (HTTP REST API). See [CLI Reference](cli.md) for flags and subcommands, and [Architecture](architecture.md) for deployment and protocol details.
...
- [CLI Reference](cli.md) — Generated command reference from `src/cli.rs`
- [Architecture](architecture.md) — Deployment patterns, RPC protocol, Provider design
